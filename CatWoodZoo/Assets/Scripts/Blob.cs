...
public class Blob : MonoBehaviour {
    ...
    private void Start()
    {
        GetComponent<MeshFilter>().mesh = Create(subdivisions, radius);
    ...
    public Mesh Create(int subdivisions, float radius)
    {
        if (subdivisions < 0)
        {
            subdivisions = 0;
    ...
    private  void CreateColours(Color[] colors)
    {
        for (int i = 0; i < colors.Length; i++)
        {
            float redTone = Colour.r + Random.Range(-ColorRange, ColorRange);
            float greenTone = Colour.g + Random.Range(-ColorRange, ColorRange);
            float blueTone = Colour.b + Random.Range(-ColorRange, ColorRange);
            colors[i] = new Color(redTone, greenTone, blueTone);
        }
    }
